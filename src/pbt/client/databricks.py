...
class DatabricksClient:
    ...
    def upload_src_path(self, src_path: str, destination_path: str):
        ...
        if destination_path.startswith("/Workspace/"):
            DBRequests(self.host, self.token, self.headers).put_workspace_file(src_path, destination_path, overwrite=True)
...
class PermissionsApi(object):
    ...
    def patch_job(self, scheduler_job_id: str, data, headers=None):
        return self.client.perform_query("PATCH", f"/permissions/jobs/{scheduler_job_id}", data=data, headers=headers)
