...
class RestClientFactory:
    ...
    def _get_fabric_info(self, fabric_id: str) -> FabricInfo:
        if fabric_id is None or fabric_id == "":
            ...
            if fabric_info is None:
                raise ValueError(
                    f"Fabric Id {fabric_id} is not found in the fabric configs {self.fabric_config}")
            return fabric_info
    ...
    def s3_client(self, fabric_id: str) -> S3Client:
        ...
        try:
            ...
            if emr is not None:
                client = S3Client(emr.region, emr.access_key_id, emr.secret_access_key,
                                  emr.session_token)
    ...
    def airflow_client(self, fabric_id: str) -> AirflowRestClient:
        ...
        if composer is not None:
            client = ComposerRestClient(composer.airflow_url, composer.project_id,
                                        composer.client_id,
