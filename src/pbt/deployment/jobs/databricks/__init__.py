...
class DatabricksJobsDeployment:
    ...
    def _reset_and_patch_job(self, job_id, job_info):
        ...
        external_id = job_info.external_job_id
        ...
        try:
            ...
            if job_data.acl:
                log_success(f"Refreshed job {job_id} with external job id {external_id} patching job acl")
                try:
                    client.patch_job_acl(external_id, job_data.acl)
                except Exception as e:
                    log_error(f"Error patching job acl for job {job_id} in fabric {fabric_name}", e)
                    return Either(left=e)
            else:
                log_success(f"Refreshed job {job_id} with external job id {external_id}, no acl found")
            ...
            if e.response.status_code == 400 and f"Job {external_id} does not exist." in response:
                log_success(
                    f"Job {job_id} external_job_id {external_id} deleted in fabric {fabric_name}, trying to recreate it")
                return self._deploy_add_job(job_id, self.valid_databricks_jobs.get(job_id),
                                            step_id=self._REFRESH_JOBS_STEP_ID)
            else:
                log_error(
                    f'Error while refreshing job with scheduler id {external_id} and id {job_id} for fabric {fabric_name}',
                    e)
                return Either(left=e)

        except Exception as e:
            log_error(
                f'Error while resetting job with {external_id} and job-id {job_id} for fabric {fabric_name}',
                e)
            return Either(left=e)
    ...
    def _delete_job(self, job_info: JobInfo):
        ...
        log(f"Trying to delete job with scheduler id {external_id} and id {job_id} for fabric {fabric_name}",
            level=LogLevel.TRACE)
        if fabric_id is None:
            msg = f"Invalid fabric {fabric_name} not deleting job {job_id} and databricks id {external_id}"
        ...
        try:
            client = self.get_databricks_client(fabric_id)
            client.delete_job(external_id)
            log_success(
                f'Successfully deleted job with scheduler id {external_id} and id {job_id} for fabric {fabric_name}')
            ...
            if e.response.status_code == 400 and "does not exist." in response:
                ...
                return Either(right=JobInfoAndOperation(job_info, OperationType.DELETED))
            log_error(
                f'Error while deleting job with scheduler id {job_info.external_job_id} and id {job_info.id} for fabric {fabric_id}',
                e)
