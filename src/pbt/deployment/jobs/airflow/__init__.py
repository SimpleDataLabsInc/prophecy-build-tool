...
class AirflowJobDeployment:
    ...
    def __init__(self, project: Project, project_config: ProjectConfig):
        ...
        self.config_override = project_config.configs_override
    ...
    def _initialize_airflow_jobs(self):
        ...
        for job_id, parsed_job in self._project.jobs.items():
            ...
            fabric_override = str(self.config_override.find_fabric_override_for_job(job_id))
            ...
            if 'Databricks' not in parsed_job.get('scheduler', None) and \
                    self.config_override.is_job_to_run(job_id) and does_fabric_exist:
                ...
                jobs[job_id] = AirflowJob(parsed_job, prophecy_job_json, rdc, rdc_with_placeholders, sha,
                                          fabric_override=self.config_override.find_fabric_override_for_job(
                                              job_id))
    ...
    def add_job(self, job_id, job_data):
        ...
        try:
            ...
            job_info = JobInfo.create_job_info(job_data.name, job_id, job_data.fabric_id, dag_name,
...
class AirflowGitSecrets:
    ...
    def deploy(self):
        ...
        if len(self.airflow_jobs.prophecy_managed_dbt_jobs) > 0:
            ...
            update_step_state(responses, self._AIRFLOW_GIT_SECRETS_STEP_ID)
            return responses
        else:
