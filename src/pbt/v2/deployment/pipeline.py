...
class PipelineUploader:
    def __init__(self, project: Project, project_config: ProjectConfig, pipeline_id: str,
        ...
        self.rest_client_factory = RestClientFactory(project_config.state_config)
    ...
    def upload_pipeline(self):
        try:
            ...
            for fabric_id in self.all_fabrics:
                fabric_info = self.project_config.state_config.get_fabric(fabric_id)
                ...
                if db_info is not None:
                    ...
                    log(f"Uploading pipeline to databricks from-path {upload_path} to to-path {upload_path} for fabric {fabric_id}",
                    ...
                    client.upload_file(emr_info.bucket, upload_path, self.from_path)
                    ...
                    if self.project.project_language == "python":
                        ...
                        client.upload_content(emr_info.bucket, launcher_path, content)
                        log(f"Uploading pipeline to s3 from-path {upload_path} to to-path {upload_path} for fabric {fabric_id}",
                            step_id=self.pipeline_id)
