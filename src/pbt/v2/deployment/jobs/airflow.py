...
class AirflowJobDeployment:
    ...
    def _pause_jobs(self) -> Dict[str, AirflowJob]:
        ...
        disabled_jobs_where_old_was_enabled = {
            ...
            if any(
                airflow_job.id == job_id and airflow_job.is_paused is True
                for airflow_job in self._deployment_state.get_airflow_jobs
            )
        }
    ...
    def _add_job(self, job_id, job_data):
        ...
        try:
            ...
            try:
                ...
                log(f"Successfully unpaused dag {dag_name}", step_id=self._ADD_JOBS_STEP_ID)
            except Exception as e:
                log(f"Failed to pause dag with name {dag_name}", exception=e, step_id=self._ADD_JOBS_STEP_ID)
    ...
    def _delete_job(self, jobs_info: JobInfo):
        try:
            ...
            log(f"Failed to delete dag for job_id: {jobs_info.id}", exception=e, step_id=self._DELETE_JOBS_STEP_ID)
            return Either(left=e)
    ...
    def _pause_job(self, job_id: str, job_data: AirflowJob):
        ...
        try:
            ...
            job_info = self._deployment_state.get_job(job_id, job_data.fabric_id)
