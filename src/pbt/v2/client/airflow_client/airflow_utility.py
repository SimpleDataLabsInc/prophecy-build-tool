...
from src.pbt.v2.exceptions import UnknownAirflowProvider, FabricNotConfiguredException
...
def create_airflow_client(fabric_id: str, state_config_and_db_tokens: StateConfigAndDBTokens):
    ...
    if fabric_info is not None and fabric_info.content is not None:
        ...
        if fabric_info.content.composer_info is not None:
            ...
            raise UnknownAirflowProvider(f"unknown provider")
        ...
        def wrapper(*args, **kwargs):
            ...
            while retries > 1:
                try:
                    ...
                    if retries < 1:
                        raise e
                    print(f"{str(e)}, Retrying in {delay_in_seconds} seconds...")
